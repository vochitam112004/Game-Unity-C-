...
#elif UPPipeline
        var lightURP = LightPropertiesTarget.InstanceURP;
        if (lightURP)
        {
            OverridePropertiesWithURPData(master, light, lightURP);
            return;
        }
#endif
...
#if UPPipeline
    void OverridePropertiesWithURPData(PropertyMaster master, Light light, UniversalAdditionalLightData additionalLightData)
    {
        if (rotation.overrideState)
            light.transform.localRotation = Quaternion.Euler(rotation.value);
